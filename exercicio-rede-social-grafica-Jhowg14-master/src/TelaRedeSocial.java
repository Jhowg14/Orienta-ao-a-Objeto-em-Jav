...
public class TelaRedeSocial {
    ...
    // comboBox para selecionar o autor do feed
    ...
    */
    ...
    private void criarComponentes() {
        ...
        areaTextoFeed.setFont(new Font("Calibre", Font.ITALIC, 16));
        ...
        comboBoxAutor.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                //garantir que a atualização do feed não será feita enquanto os itens estão sendo recarregados (dica: use um atributo boolean para isso).
                if(bol) {
                    atualizarAreaTextoFeed();
                }
            }
    ...
    private void montarJanela() {
        ...
        JScrollPane Pane = new JScrollPane(areaTextoFeed);
        JPanel painelCentral = new JPanel();
        painelCentral.setLayout(new BoxLayout(painelCentral, BoxLayout.Y_AXIS));
        painelCentral.add(new JLabel("Feed de Notícias"));
        painelCentral.add(Pane);
       ...
       // Colocar a caixa de seleção dentro de um painel que esteja na área superior (NORTH) do BorderLayout.
        ...
        painelSuperior.add(comboBoxAutor);
    ...
    */
    ...
    private void curtirMensagem() {
        ...
        //continue pedindo o identificador para o usuário até ele digitar um identificador válido.
        ...
        do{
            try {
                ...
                if(feed.getAutores().isEmpty()) {
                    JOptionPane.showMessageDialog(janela, "Não há mensagens para curtir", "Erro", JOptionPane.ERROR_MESSAGE);
                    deuCerto = true;
                }else{
                    JOptionPane.showMessageDialog(janela, "Mensagem com ID " + e.getId()+" não encontrada", "Erro", JOptionPane.ERROR_MESSAGE);
                }
            }
        }while(!deuCerto);
    }    
    private void comentarMensagem() {
        int idMensagem = Integer.parseInt(JOptionPane.showInputDialog("Id da mensagem"));
        String mensagem = JOptionPane.showInputDialog("Texto do comentario");  
    ...
    private void comboBoxAutor() {
        ...
        comboBoxAutor.addItem("Todos");
       //Transfere o conteúdo do ArrayList em forma de String para o JComboBox
     ...
     */
    //Alterar o método atualizarAreaTextoFeed para que ele carregue o feed de acordo com a escolha do usuário na caixa de seleção
    // dica: você pode obter a opção escolhida pelo usuário usando os métodos getItemAte getSelectedIndex da classe JComboBox)
    private void atualizarAreaTextoFeed() {  
        ...
        //carregue o feed de acordo com a escolha do usuário na caixa de seleção
        if (comboBoxAutor.getSelectedIndex() == 0) {
            ...
            for (Publicacao publicacao : publicacoes) {
                if (publicacao.getAutor().equals(comboBoxAutor.getSelectedItem())) {
                    areaTextoFeed.append(publicacao.getTextoExibicao());
                }
            }
        }
