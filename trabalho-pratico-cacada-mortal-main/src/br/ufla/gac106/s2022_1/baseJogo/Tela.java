...
public class Tela implements InterfaceUsuario
{
    ...
    private void criarPainelSuperior(String nomeJogo) {
        ...
        painelSuperior.setPreferredSize(new Dimension(larguraJanela,100));
        
        // Cria e adiciona um rótulo com a identificação do jogo
        criarSubPaineltitulo(painelSuperior,nomeJogo);
        criarPainelTempo(painelSuperior);
    ...
    private void criarSubPaineltitulo(JPanel painelPai, String nomeJogo){
        JPanel painelTitulo = new JPanel();
        painelTitulo.setPreferredSize(new Dimension(larguraJanela,50));
        ...
        painelPai.add(painelTitulo);
    }
    private void criarPainelTempo(JPanel painelPai){

        JPanel painelTempo = new JPanel();
        painelTempo.setPreferredSize(new Dimension(larguraJanela,40));

        horaInicio = System.currentTimeMillis();
        tempoJogo = new JTextArea(); 
        tempoJogo.setFont(new Font("Consolas", Font.BOLD, 20));
        tempoJogo.setMargin(margem);
        tempoJogo.setEditable(false);
        tempoJogo.setBackground(janela.getBackground());
        painelTempo.add(tempoJogo);
        JButton botaoAtualizar = new JButton("Atualizar");
        painelTempo.add(botaoAtualizar);
        painelPai.add(painelTempo);

        Action acaoAtualizar = new AbstractAction()
        {
            @Override
        ...
        botaoAtualizar.addActionListener(acaoAtualizar);
    ...
    public void exibirTempo() {
        ...
        long diferenca = (tempoAtual - horaInicio);

        long diferencaSeg = diferenca/1000;
        long diferencaMin = diferencaSeg/60;
        
        if (diferencaMin > 0){
            diferencaSeg = diferencaSeg/diferencaMin;
        }
        ...
        if (diferencaMin < 1) {
            tempo = "Há " + diferencaSeg + " segundos";
        }
        else {
            if(diferencaSeg < 10){
                tempo = "Há " + diferencaMin + ":" + "0" + diferencaSeg;
            }
            else{
                tempo = "Há " + diferencaMin + ":" + diferencaSeg;
            }
        }
